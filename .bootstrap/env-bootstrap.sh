...
if [[ -z "$ENVIRONMENT" || "$ENVIRONMENT" == "default" ]]; then
  echo "❌ Must have a non-default Terraform workspace selected (e.g., dev, staging, prod)."
  exit 1
fi
...
echo "✅ tfvars files downloaded to local environment."
...
echo "🚀 Running terraform plan for environment: \$ENVIRONMENT in region: \$AWS_REGION"
...
echo "🚀 Running terraform apply for environment: \$ENVIRONMENT in region: \$AWS_REGION"
...
echo "✅ plan.sh and apply.sh created in /infra using current Terraform workspace: $ENVIRONMENT"
